...
#  الإعدادات
# ================================================================
BOT_TOKEN   = "8967873747:AAEuiG8o_MbiYr77HiVkBwSG52oy7GWrpgk"
ADMIN_ID = 5179688953 # حط هنا الايدي نتاعك
MAX_HISTORY = 12
API_URL     = "https://zeneath.vectorion.in/app/newapi/api2.php"
GPT55_URL = "http://de3.bot-hosting.net:21007/kilwa-chatgpt?text="
...
def get_user(uid):
    with state_lock:
        ...
        # كل دخول (/start)
        user_state[uid]["joins"] += 1
        user_state[uid]["last_join"] = time.time()
...
def call_model_full(model_key, messages, timeout=None):
         # ✅ ChatGPT 5-5 special API (خارج API_URL العادي)
    if model_key == "gpt-5-5":
        try:
            user_text = messages[-1]["content"]
            url = "http://de3.bot-hosting.net:21007/kilwa-chatgpt?text=" + requests.utils.quote(user_text)
            r = requests.get(url, timeout=60)
            return r.text
        except Exception as e:
            return f"[GPT-5-5 error] {e}"
    """
...
def cmd_start(message):
    ...
    # ⏱️ وقت الدخول
    join_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(user["last_join"]))
    first_time = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(user["first_join"]))
    ...
    bot.send_message(
        ...
        f"📛 اليوزر: @{username}\n"
        f"🆔 الايدي: {uid}\n\n"
        f"📊 عدد الدخولات: {user['joins']}\n"
        f"⏰ أول دخول: {first_time}\n"
        f"🔄 آخر دخول: {join_time}"
    )

    # 👇 نفس رسالتك 100% بدون تغيير
    bot.send_message(
        message.chat.id,
        f"👋 أهلاً {name}!\n\n"
        f"😈 Lucifer AI — FUSION ENGINE\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n"
